...
protected:
    ...
    bool Check_user(std::string user_name, std::string user_password);
    ...
    map<string, string> connUUID;
    map<string, std::shared_ptr<OneClient>> uuidClient;

    /* REFACTORING */
    typedef std::string connection_id;
    typedef std::string user_id;
    std::unordered_map<connection_id, user_id> m_connectionToUUID; // probably not needed
