...
bool WebsocketServer::Check_user(std::string user_name,std::string user_password)
{
    if(user_name.size() == 0)
        return false;
...
void WebsocketServer::commandSendFile(const ClientConnection& conn, Json& options)
{
    ...
    if (!path_from_file.empty() && std::filesystem::exists(path_from_file) && !std::filesystem::is_directory(path_from_file))
    {
        char key[] = ___key;
...
void WebsocketServer::onOpenInfo(const ClientConnection& conn)
{
    ...
    std::string uuid = std::to_string(uuidClient.size());
        ...
        switch (type_command) {
            ...
            case EClientMessageType::START_SEND_BIG_FILE:
            {
                ...
                // sendMessageInfo(client->m_connInfo, EServerMessageType::FILE_SENT, newJson);
                {
                    ...
                    client->StartFile(tfile, secret, parents_hashs, file_size);
                }
            } break;
            ...
            {
                std::string files = "";
                auto list = FileMapToVector(&client->m_files);
    ...
    if (msgJson->IsObject() && !msgJson->HasParseError() && msgJson->HasMember(MESSAGE_FIELD)) {
        ...
        switch (type_command) {
            ...
            case EClientMessageType::FILE_CHUNK_INFO: {
                ...
                {
                    std::lock_guard<std::mutex> l(client->m_mutex);
                    client->WriteChunkToCurrentFile((uint8_t *)msg->get_raw_payload().data() +
                                                        std::strlen(msg_strig.data()) + 1,
                                                    offset, 64 * 1024);
                }
            } break;
            ...
            case EClientMessageType::END_SEND_BIG_FILE:
            {
                ...
                {
                    std::lock_guard<std::mutex> l(client->m_mutex);
                    client->EndFile(file, secret, parents_hashs, 0);
            ...
            case EClientMessageType::MERGE_SOCKETS_STEP_1: {
                ...
                auto conn_id = GetConnectionId(conn);
                m_connectionToUUID[conn_id] = uuid;
